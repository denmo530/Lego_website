...
<div class="dropdown">
		...
		<div class="dropdown-content">
			<a href="Nyheter.php?order=DESC">Senaste 12</a>
			<a href="Nyheter.php?order=ASC">Äldsta 12</a>
		</div>
	</div>
...
<?php
		...
		if($order === "ASC"){
			print("<h3 id='ASC'>Visar nu 12 äldsta seten</h3>");
		}

		else{
		print("<h3 id='ASC'>Visar nu 12 senaste seten</h3>");
		$order = "DESC";
		}

		$latestSets = mysqli_query($connection, "SELECT sets.SetID, sets.Setname, sets.Year, images.has_largegif, images.has_largejpg, images.has_jpg, images.has_gif 
		FROM sets, images
		WHERE sets.Year AND images.ItemtypeID='S' AND images.ItemID=sets.SetID ORDER BY sets.Year $order LIMIT 12");

...
<?php
		...
		print("<h3 class='currentPart'>Currently showing all sets that contain PartID: " . $partID . "</h3>");
		

				
		$sets = mysqli_query($connection, "SELECT sets.Setname, sets.SetID, images.has_largejpg, images.has_largegif, images.has_jpg, images.has_gif FROM sets, images, inventory WHERE sets.SetID = inventory.SetID AND inventory.ItemID = '$partID' AND inventory.ColorID = '$colorID' AND images.ItemID = sets.SetID");
